...
export const plugin: PluginFunction = (_schema, documents, _config, info) => {
  const outputDir = dirname((info && info.outputFile) || process.cwd());
  ...
  for (const { selection, path } of imports) {
    lines.push(`import ${selection} from '${relative(outputDir, path)}';`);
  }
